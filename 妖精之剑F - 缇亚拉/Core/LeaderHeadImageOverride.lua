...
if not MapModData.g_LeaderHeadImageOverrideInit then MapModData.g_LeaderHeadImageOverrideInit = false end
if MapModData.g_LeaderHeadImageOverrideInit == true then 
	return
else
	MapModData.g_LeaderHeadImageOverrideInit = true
...
Events.LoadScreenClose.Add(
	function()
		MapModData.g_LeaderHeadImageOverrideInit = false

		local screenX, screenY = UIManager:GetScreenSizeVal()
		local contexts = {}
		local approaches = {
			[MajorCivApproachTypes.MAJOR_CIV_APPROACH_HOSTILE]	= "LeaderSceneOverrideHostile",
			[MajorCivApproachTypes.MAJOR_CIV_APPROACH_GUARDED]	= "LeaderSceneOverrideGuarded",
			[MajorCivApproachTypes.MAJOR_CIV_APPROACH_AFRAID]	= "LeaderSceneOverrideAfraid",
			[MajorCivApproachTypes.MAJOR_CIV_APPROACH_FRIENDLY] = "LeaderSceneOverrideFriendly",
			[MajorCivApproachTypes.MAJOR_CIV_APPROACH_NEUTRAL]	= "LeaderSceneOverride"
		}
			...
			function( iPlayer, iDiploUIState, szLeaderMessage, iAnimationAction, iData1 )
				...
				if Game.IsNetworkMultiPlayer() then
					...
					if chosenTexture == nil and chosenTexture ~= "null" then   --need the ~= "null" check since when running as an MP modpack, null is actually returned as a string by the DB
						...
						if (playerLeaderInfo.LeaderSceneOverrideDefeat and (player:IsAlive() == false)) then
							chosenTexture = playerLeaderInfo.LeaderSceneOverrideDefeat
						elseif (playerLeaderInfo.LeaderSceneOverrideWar and (bWar == true)) then
							chosenTexture = playerLeaderInfo.LeaderSceneOverrideWar
						elseif (playerLeaderInfo.LeaderSceneOverrideDenouncing and (player:IsDenouncingPlayer(Game:GetActivePlayer()))) then
							chosenTexture = playerLeaderInfo.LeaderSceneOverrideDenouncing
